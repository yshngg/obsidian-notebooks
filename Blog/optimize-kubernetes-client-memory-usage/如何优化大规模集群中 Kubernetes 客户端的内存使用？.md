...
在扩展 Kubernetes 集群功能时，我们需要用到客户端库 [k8s.io/client-go](https://pkg.go.dev/k8s.io/client-go) 编码客户端与 Kubernetes API server 通信，List/Watch 资源，做一些事情。比如 [kube-state-metrics](https://github.com/kubernetes/kube-state-metrics)，List/Watch Pod/Deployment/Job/Daemonset/Job 等资源，基于资源信息生成 Prometheus 指标，供 Prometheus 应用程序抓取。再比如 [Prometheus Operator](https://github.com/prometheus-operator/prometheus-operator) List/Watch ServiceMonitor/PodMonitor 等资源更新 [Prometheus 配置文件](https://prometheus.io/docs/prometheus/latest/configuration/configuration/#configuration-file) 以采集指标。

客户端库 [k8s.io/client-go](https://pkg.go.dev/k8s.io/client-go) 使用 [SharedIndexInformer](https://pkg.go.dev/k8s.io/client-go@v0.36.0/tools/cache#SharedIndexInformer) 缓存 Kubernetes 集群中资源到内存中以减少对 Kubernetes API server 的请求数量，从而减轻 Kubernetes API server 和 etcd 的 CPU/内存压力和减少网络流量。问题在于 [SharedIndexInformer](https://pkg.go.dev/k8s.io/client-go@v0.36.0/tools/cache#SharedIndexInformer) 运行后首先 List 集群中所有的资源并缓存到内存中，再 Watch 资源变化更新本地缓存。如果集群中有大量资源，就算 Kubernetes API server 及时响应了客户端（如果请求的数据量很大， Kubernetes API server 响应速度很慢，有可能会超时），全部缓存到内存中，也会导致客户端程序启动时内存用量暴涨。此时，客户端工作负载的资源 limit 配置不当或所在节点物理硬件资源不足，就会发生 OOM[^1] 。

## 如何做

作为客户端开发者，我们应该优化代码，降低内存使用，保障服务稳定运行并节省成本。在我阅读了一些 Kubernetes 组件源码后，我总结了下面这几种手段。
